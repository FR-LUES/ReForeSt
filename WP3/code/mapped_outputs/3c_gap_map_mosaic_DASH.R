# List 100km tiles
OS_folders_100km <- list.dirs(path_gap_map_10km_DASH, recursive = F, full.names = F)

# Mosaic each OS 100km tile
map(1:length(OS_folders_100km), .f = function(x) {
  
  mosaicFunction_DASH(paste0(path_gap_map_10km_DASH, OS_folders_100km[[x]]),
                      path_gap_map_100km_DASH,
...
path_fyl <- paste0(path_gap_map_fyl_DASH, "fylingdales_VOM_gaps.tif")
fyl <- rast(path_fyl)
...
for (t in c("NZ", "SE", "TA")) {
  ...
  file <- paste0(path_gap_map_100km_DASH, t, "_VOM_gaps.tif")
  r <- rast(file)
  ...
  # Copy to DBFS
  file_out <- paste0(path_gap_map_100km_DASH, t, "_VOM_gaps_clip.tif")
...
tiles <- list.files(path_gap_map_100km_DASH, pattern = "\\.tif$", full.names = TRUE)
...
# Create VRT 
vrt <- vrt(tiles, paste0(path_gap_map_eng_DASH, "gap_map_1m_preclip.vrt"), overwrite = TRUE)
...
# Copy to DBFS
file.copy(localPath, paste0(path_gap_map_eng_DASH, "gap_map_1m_preclip.tif"), overwrite = TRUE)
file.remove(localPath)
  

# Clip to England
england <- vect(path_eng_DASH) %>% terra::aggregate() 
england_VOM_gaps <- rast(paste0(path_gap_map_eng_DASH, "gap_map_1m_preclip.tif"))
...
# Copy to DBFS
file.copy(localPath, path_gap_map_DASH, overwrite = TRUE)
file.remove(localPath)